...
const { promoteAdmin } = require('./models/User');
const token = process.env.BOT_TOKEN;
const client = new WebClient(token);
const SLACK_ID_REGEX = /^!promote <@[UW][A-Z0-9]{10}>$/g;
...
bot.message(async ({ message, say }) => {
  ...
  console.log(user);
  console.log('not including !promote!!!!', message);

  // if a message.text matches regex pattern
  if (message.text.match(SLACK_ID_REGEX)) {
    console.log('message in if statement', message.text.match(SLACK_ID_REGEX));
